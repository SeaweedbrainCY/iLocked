...
class AddKey: UIViewController, UITextViewDelegate,UIScrollViewDelegate, UITextFieldDelegate  {
    ...
    @objc private func newKeyWithQRCode(notification: Notification) {
        ...
        if qrCodeString == nil {
            perform(#selector(showErrorView), with: "Impossible to retrieve any data of the QRCode", afterDelay: 0.5)
            queue.async {
            ...
            do{
                ...
                if data[QRCodeData.QRCodeData.type.key] == QRCodeData.QRCodeData.type.pubKey {
                    ...
                    perform(#selector(showErrorView), with: "The QRCode doesn't contain a public key", afterDelay: 0.5)
                    queue.async {
                ...
                switch errorThrown{
                case .incorrectJSON:
                    perform(#selector(showErrorView), with: "Invalid data. Cannot be read", afterDelay: 0.5)
                case .unknownVersion:
                    perform(#selector(showErrorView), with: "Invalid QRCode version. Cannot be read", afterDelay: 0.5)
                case .invalideChesksum:
                    perform(#selector(showErrorView), with: "The QRCode data is corrupted", afterDelay: 0.5)
                case .unknownType:
                    perform(#selector(showErrorView), with: "Data type unknown. Impossible to decode", afterDelay: 0.5)
                case .noChecksum:
                    perform(#selector(showErrorView), with: "The data integrity cannot be checked. Decoding prohibited", afterDelay: 0.5)
                case .noKey:
                    perform(#selector(showErrorView), with: "The QRCode doesn't contain a public key", afterDelay: 0.5)
                }
                queue.async {
                ...
                //alert("Invalid QRCode ! ", message: "Make sure your QRCode has been generated by iLocked.")
                perform(#selector(showErrorView), with: "Invalid format. Cannot be read", afterDelay: 0.5)
            }
        }
    }
