...
class AddKey: UIViewController, UITextViewDelegate,UIScrollViewDelegate, UITextFieldDelegate  {
    ...
    @objc private func showErrorView(withTitle title : String ){
        self.notificationView.setTitle(title, for: .normal)
    ...
    @objc private func newKeyWithQRCode(notification: Notification) {
        ...
        if qrCodeString == nil {
            perform(#selector(showErrorView), with: "Impossible to retrieve any data of the QRCode", afterDelay: 1)
            queue.async {
            ...
            do{
                ...
                if data[QRCodeData.QRCodeData.type.key] == QRCodeData.QRCodeData.type.pubKey {
                    ...
                    perform(#selector(showErrorView), with: "The QRCode doesn't contain a public key", afterDelay: 1)
                    queue.async {
                ...
                perform(#selector(showErrorView), with: "Invalid format. Cannot be read", afterDelay: 1)
